...
_default_config_name = "Workflow Block "
...
def get_sample_name(idx):
    global __sample_setup__
    return __sample_setup__.get_sample_name(idx)

def get_sample_thickness(idx):
    global __sample_setup__
    return __sample_setup__.get_sample_thickness(idx)

def get_enabled_sample_idx():
    global __sample_setup__
...
def run_scan():
    ...
    try:
        path = get_project_path('Local') + '/gumtree_autosave'
...
act_add.colspan = 2
act_run = Act('run_scan()', 'Run GSOM Scan')
