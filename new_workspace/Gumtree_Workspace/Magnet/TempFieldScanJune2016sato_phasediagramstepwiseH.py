...
def scan_temp_theta_N(fieldList, mode, preset, tempSettleTime = 1, thetaSettleTime = 1):
#    for wavelength in wavelengthList:
    log('driving magnet1_driveable=' + str(0))
    sics.drive('ma1_setpoint', 0) 
    log('driving tc1_driveable=' +  str(60))
    sics.drive('tc1_driveable', 60)
    log('driving magnet1_driveable=' + str(190))
    sics.drive('ma1_setpoint', 190) 
    log(str(57.5))
    sics.drive('tc1_driveable', 57.5)
    log('waiting for temperature to settle')
    sleep(900)
...
def scan_field_theta_N(fieldList, mode, preset, tempSettleTime = 1, thetaSettleTime = 1):
#    for wavelength in wavelengthList:
    for i in xrange(len(fieldList)):
        log('driving magnet1_driveable=' + str(0))
        sics.drive('ma1_setpoint', 0) 
...
tempFieldList = [
                 ...
                 (57.125, [0,25,50,75,100,125,150,175,200,225,250,275,300,325,350,375,400]),
...
#scan_temp_theta_N(tempthetaList, mode, preset_thet, tempSettleTime_thet, thetaSettleTime)
#scan_field_theta_N(fieldthetaList, mode, preset_thet, tempSettleTime_thet, thetaSettleTime)
