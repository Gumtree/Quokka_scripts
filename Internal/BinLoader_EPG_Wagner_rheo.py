...
class QuokkaBinLoader:
    ...
    def exportDatasets(self, nexusReference = None):
        ...
        ref.__iDictionary__.addEntry('data_counts', '$entry/data/total_counts' )
        ref.__iDictionary__.addEntry('total_counts', '$entry/instrument/detector/total_counts' )
        # monitor
        ...
        ref.__iDictionary__.addEntry('bm_time'    , '$entry/monitor/time'  )
        ref.__iDictionary__.addEntry('detector_time'    , '$entry/instrument/detector/time'  )
        # beam time
...
###EPG INSERTS HERE###
loader = QuokkaBinLoader(r'Z:\cycle\167\data\histserv\DAQ_2025-11-29T07-44-42\DATASET_0\EOS.bin', 10800)
...
loader.setCenter(centerX = 98.7207, centerY = 95.0203) # center dose not affect resulting nexus-files, this is only for visualization within GumTree
...
#loader.setEqualTimeBins(120)                 # 600 seconds time bins
###EPG ADDED SECTIONS HERE###
#loader.setManualTimeBins([0,1750,1760,3550,3560,5350,5360,7150,7160,8950,8960,10750,10760,12000]) # new bin at 500, 3000 and 5000 seconds
#loader.setManualTimeBins([0,700,710,1000,1010,1300,1310,1600,1610,1900,1910,2200,2210,2510,2810,3110,3410,3710,4010,4310,4610,4910,5210,5510,5810,6110,6410,6710]) # 282787
#loader.setManualTimeBins([0,820,830,1120,1130,1420,1430,1720,1730,2020,2030,2320,2330,2630,2930,3230,3530,3830,4130,4430,4730,5030,5330,5630,5930,6230,6530,6830]) # 282791
loader.setManualTimeBins([160,1050,1060,1350,1360,1650,1660,1950,1960,2250,2260,2550,2560,2860,3160,3460,3760,4060,4360,4660,4960,5260,5560,5860,6160,6460,6760]) # 282795
#loader.setManualTimeBins([150,1040,1050,1340,1350,1640,1650,1940,1950,2240,2250,2540,2550,2850,3150,3450,3750,4050,4350,4650,4950,5250,5550,5850,6150,6450,6750]) # 282799
loader.setManualTimeBins([90,980,990,1280,1290,1580,1590,1880,1890,2180,2190,2480,2490,2780,2790,3080,3090,3380,3390,3690,3990,5290,5590,5890,6190,6490,6790,7090,7390,7690]) # 282803
...
loader.exportDatasets(r'W:\data\proposal\00105\2025\21680_Wagner\EVENT MODE DATA\QKK0082803.nx.hdf')
